...
from .server_app import ObjaverseRetrievalApp

console_logger = logging.getLogger(__name__)


class ObjaverseRetrievalServer:
    """
    Manages the lifecycle of an Objaverse retrieval server with proper resource
    management and clean shutdown capabilities.

    Unlike the BlenderServer, this doesn't need subprocess isolation since
    Objaverse retrieval with CLIP doesn't have the same thread-safety requirements
    as bpy. The server runs Flask in a separate thread within the same process,
    ...
    Example:
        >>> server = ObjaverseRetrievalServer(host="127.0.0.1", port=7007)
        >>> server.start()
        >>> server.wait_until_ready()
        >>> # ... use server via ObjaverseRetrievalClient ...
        >>> server.stop()
    """
    ...
    def __init__(
        ...
        """Initialize the Objaverse retrieval server manager.

        Args:
            host: The host address to bind the server to.
            port: The port number to bind to (default: 7007).
            preload_retriever: Whether to preload the Objaverse retriever (includes
                CLIP model loading) on server start. When True, the retriever is
                loaded during initialization, eliminating first-request latency.
                When False, the retriever is loaded lazily on first request.
                Default: True.
        ...
        self._app: ObjaverseRetrievalApp | None = None
        ...
        console_logger.debug(
            f"Initialized ObjaverseRetrievalServer(host={host}, port={port}, "
            f"preload_retriever={preload_retriever}, clip_device={clip_device})"
        )

    def start(self) -> None:
        """Start the Objaverse retrieval server.
        ...
        console_logger.info(
            f"Starting Objaverse retrieval server on {self._host}:{self._port}"
        )

        try:
            # Create the Flask application.
            self._app = ObjaverseRetrievalApp(
                preload_retriever=self._preload_retriever,
                objaverse_data_path=self._objaverse_data_path,
                objaverse_preprocessed_path=self._objaverse_preprocessed_path,
                objaverse_top_k=self._objaverse_top_k,
            ...
            console_logger.info(
                f"Objaverse retrieval server ready on {self._host}:{self._port}"
            )
            console_logger.info(
    ...
    def stop(self) -> None:
        """Stop the Objaverse retrieval server gracefully."""
