...
`yz` 内嵌 lookup 现为 10,963 个 HSSD 资产全部提供 `self_emission`：998 个
灯具、63 个发光显示设备、129 个火焰类资产、9,773 个非发光资产。字段区分
电功率 W、光通量 lm、发光强度 cd 和显示面亮度 cd/m2，并保留类别估计、真实
GLB 材质扫描和来源。轻量查询可用
`get_self_emission_annotations()` / `search_self_emission()`，不会触发外部净空与
affordance enrichment。完整方法、公式和限制见
[`HSSD_SELF_EMISSION_ANNOTATIONS.md`](HSSD_SELF_EMISSION_ANNOTATIONS.md)。
