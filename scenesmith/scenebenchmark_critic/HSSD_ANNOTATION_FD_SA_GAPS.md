...
scene-instance ids and must be resolved by SceneSmith at runtime.
...
The `yz` branch now fills these gaps by adding `scenebenchmark_fd_sa` and
`scenebenchmark_functional_hints` to every bundled HSSD lookup record and by
hydrating those fields into SceneBenchmark adapter functional hints at runtime.
